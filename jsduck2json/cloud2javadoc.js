...
function toAndroidType(str) {
	var tokens = str.split('/');
	if (tokens.length > 1) {
		var rv = [];
		tokens.forEach(function (token) {
			rv.push(toAndroidType(token));
		});
		return rv.join('/');
	}

	if (~str.indexOf('[]')) {
		return toAndroidType(str.replace('[]', '')) + '[]';
	}
...
function parseMethods(members) {
	...
	members.forEach(function(api) {
		...
		if ('shortDoc' in api) {
			desc += shortenDesc(api.shortDoc) + '<br/><br/>\n';
		} else {
			desc += shortenDesc(api.doc) + '<br/><br/>\n';
		}
		...
		desc += '     * <a href="http://docs.appcelerator.com/arrowdb/latest/#!/api/' + owner + '-method-' + name + '">ArrowDB API Docs</a>.\n';
